...
type InstaDeliveryEntryWithItems = Prisma.InstaDeliveryEntryGetPayload<{
  include: { items: true };
}>;

type ProductSnapshot = Prisma.ProductGetPayload<{}>;

export const instadeliveryEntriesRouter = {
  list: protectedProcedure
    .output(z.array(entryOutput))
    .handler(async () => {
      const entries = await prisma.instaDeliveryEntry.findMany({
        include: { items: true },
        orderBy: { date: "desc" },
      });
  ...
  create: protectedProcedure
    ...
    .handler(async ({ input }) => {
      ...
      const entry: InstaDeliveryEntryWithItems = await prisma.instaDeliveryEntry.create({
        data: {
          date: input.date,
  ...
  update: protectedProcedure
    ...
    .handler(async ({ input }) => {
      ...
      const entry = await prisma.$transaction(async (tx) => {
        await tx.instaDeliveryEntry.update({
          where: { id: input.id },
          data: {
            date: input.date,
            customerName: input.customerName.trim(),
        ...
        await tx.instaDeliveryEntryItem.deleteMany({
          where: { entryId: input.id },
        });

        await tx.instaDeliveryEntryItem.createMany({
          data: itemsData,
        });

        return tx.instaDeliveryEntry.findUnique({
          where: { id: input.id },
          include: { items: true },
        });
      });
  ...
  delete: protectedProcedure
    ...
    .handler(async ({ input }) => {
      ...
      await prisma.instaDeliveryEntry.delete({
        where: { id: input.id },
      });
